...
EasyCoder session. The manager inspects and steers that session with `easycoder-agent`.
...
- Source the current project EasyCoder environment before running `easycoder-agent`.
- Spawn engineer sessions with `easycoder-agent`.
...
- Monitor progress, ask follow-up questions, and challenge weak claims through
  the engineer's EasyCoder session.
...
1. Control plane: shared EasyCoder account/context where the manager can spawn,
   inspect, message, and review engineer sessions with `easycoder-agent`.
2. Code plane: the engineer's assigned git worktree where code changes happen.
3. Validation plane: the engineer's worktree-local EasyCoder environment created
...
- `easycoder-agent` is orchestrator-only. Engineers do not need to know about it or
  use it.
...
- All feedback to engineers goes through their EasyCoder sessions.
...
- Do not accept toy-path validation when the product behavior is meant to hold
  up under long, realistic EasyCoder usage.
...
- Report back only through this EasyCoder session
