...
- **Persistence/config:** `src/persistence.ts` + `src/configuration.ts` manage local state in `~/.easycoder`.
...
```mermaid
flowchart TD
    Start([easycoder ...]) --> Parse[Parse subcommand]
...
graph LR
    ...
    subgraph "Environment Overrides"
        direction TB
        E1[EASYCODER_HOME_DIR]
        E2[EASYCODER_SERVER_URL]
        E3[EASYCODER_WEBAPP_URL]
        E4[EASYCODER_VARIANT]
        E5[EASYCODER_EXPERIMENTAL]
        E6[EASYCODER_DISABLE_CAFFEINATE]
    end

    E1 -.-> settings & access & daemon & logs
```

Local state lives under `~/.easycoder` (or `EASYCODER_HOME_DIR`):
...
- `EASYCODER_SERVER_URL` and `EASYCODER_WEBAPP_URL` override defaults.
- `EASYCODER_VARIANT`, `EASYCODER_EXPERIMENTAL`, `EASYCODER_DISABLE_CAFFEINATE` control behavior.
