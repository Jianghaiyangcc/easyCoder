...
Implement `docs/session-protocol.md` as the new message format for Codex sessions in the CLI, and add client-side support in `easycoder-app` to parse, normalize, and render these messages alongside existing legacy formats (output, codex, acp).
...
- ⚠️ `packages/easycoder-cli` and `packages/easycoder-app` do not define a `lint` script; verification used full test suites plus `yarn typecheck` in both packages.
...
- [x] Create `packages/easycoder-cli/src/sessionProtocol/types.ts` with:
...
- [x] Add `sendSessionProtocolMessage(envelope)` to `ApiSessionClient` in `packages/easycoder-cli/src/api/apiSession.ts`
...
- [x] Run full test suite — `yarn test` in easycoder-cli, easycoder-app
