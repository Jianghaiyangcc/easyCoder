...
| File | Change |
|------|--------|
| `packages/easycoder-app/sources/components/PermissionModeSelector.tsx` | Change both `ModelMode` and `PermissionMode` to `{ key, name, description }` struct |
| `packages/easycoder-app/sources/sync/storageTypes.ts` | Change `Session.modelMode` and `Session.permissionMode` to `string | null` (store key only) |
| `packages/easycoder-app/sources/sync/storage.ts` | Widen both `updateSessionModelMode()` and `updateSessionPermissionMode()` to accept `string` |
| `packages/easycoder-app/sources/components/AgentInput.tsx` | Accept structs for both, render from metadata or hardcoded, show `name` in UI |
| `packages/easycoder-app/sources/-session/SessionView.tsx` | Build structs from metadata for both model and mode, pass to AgentInput |
| `packages/easycoder-app/sources/sync/sync.ts` | Send model key in meta for all agent types, send permission mode key |
| `packages/easycoder-app/sources/sync/typesMessageMeta.ts` | Ensure meta schema accepts any string for permissionMode |
| `packages/easycoder-app/sources/app/(app)/new/index.tsx` | Adapt to structs for both model and mode |
...
- ⚠️ `yarn workspace easycoder-app lint` is unavailable because the package has no `lint` script
