...
## Overview
Create a portable, self-contained distribution of easycoder-server as a single Bun-compiled binary. It runs without Redis (already has in-memory event bus), uses PGlite for embedded PostgreSQL, and local filesystem for file storage. CLI provides `easycoder-server migrate` and `easycoder-server serve` commands.
...
- [ ] Add `@electric-sql/pglite`, `pglite-prisma-adapter` to easycoder-server dependencies
...
- [ ] Add `build:standalone` script to `package.json`: `bun build ./sources/standalone.ts --compile --outfile easycoder-server`
...
- [ ] Test: `./easycoder-server migrate` creates and migrates a PGlite database
- [ ] Test: `./easycoder-server serve` starts the server with PGlite

### Task 7: Verify end-to-end
- [ ] Build the binary
- [ ] Run `./easycoder-server migrate` — verify database is created in `./data/`
- [ ] Run `./easycoder-server serve` — verify server starts, health endpoint responds
...
**CLI usage:**
```
easycoder-server migrate    # Apply database migrations
easycoder-server serve      # Start the server
```
...
- Consider adding `easycoder-server init` command for first-time setup
