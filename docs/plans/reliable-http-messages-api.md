...
⚠️ No lint script is defined in `packages/easycoder-server/package.json`, so linter execution is currently not available.
...
- Update `easycoder-agent` to use `POST /v3/sessions/:id/messages` instead of Socket.IO `message` event
- Update `easycoder-agent` to poll `GET /v3/sessions/:id/messages` for receiving messages
