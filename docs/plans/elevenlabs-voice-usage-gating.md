...
- Server token route: `packages/easycoder-server/sources/app/api/routes/voiceRoutes.ts`
- Client token fetch: `packages/easycoder-app/sources/sync/apiVoice.ts`
- Voice start decision: `packages/easycoder-app/sources/realtime/RealtimeSession.ts`
- ElevenLabs client session start:
  - `packages/easycoder-app/sources/realtime/RealtimeVoiceSession.tsx`
  - `packages/easycoder-app/sources/realtime/RealtimeVoiceSession.web.tsx`

## Existing Secret Assumptions

The repo already assumes ElevenLabs API access exists on the server:

- `packages/easycoder-server/sources/app/api/routes/voiceRoutes.ts` reads `process.env.ELEVENLABS_API_KEY`.
- `packages/easycoder-server/deploy/easycoder-server.yaml` extracts `/handy-elevenlabs`.
...
1. Derive a stable pseudonymous ElevenLabs `user_id` from the EasyCoder user ID.
...
This keeps the join key stable across sessions without exposing the raw EasyCoder account ID to ElevenLabs.
...
Server authenticates JWT
  |
  +--> request.userId = EasyCoder account id
  |
...
Update `packages/easycoder-server/sources/app/api/routes/voiceRoutes.ts` to:
...
- `packages/easycoder-app/sources/realtime/types.ts`
- `packages/easycoder-app/sources/realtime/RealtimeVoiceSession.tsx`
- `packages/easycoder-app/sources/realtime/RealtimeVoiceSession.web.tsx`
- `packages/easycoder-app/sources/realtime/RealtimeSession.ts`
- `packages/easycoder-app/sources/sync/apiVoice.ts`
...
There is older server-only free-trial work in the legacy `slopus/easycoder-server` repository on branch:
