# easycoder-agent CLI Tool

## Overview
A new standalone CLI tool (`easycoder-agent`) in `packages/easycoder-agent` that acts as a dedicated client for controlling EasyCoder Coder agents remotely. Unlike `easycoder-cli` which both runs and controls agents, `easycoder-agent` only controls them — listing machines, spawning sessions on a machine, creating sessions, sending messages, reading history, monitoring state, and stopping sessions.

This is a completely separate client from `easycoder-cli`. It has its own authentication flow (account auth via QR code, same as device linking in the mobile app), its own credential storage (`~/.easycoder/agent.key`), and is written from scratch with no code sharing.

## Context
- **Existing system**: Monorepo with `easycoder-cli` (agent runtime + control), `easycoder-server` (Fastify + PostgreSQL + Redis), `easycoder-app` (React Native mobile)
- **Server API**: REST endpoints at `https://codeapi.daima.club` + Socket.IO at `/v1/updates`
- **Authentication**: Uses account auth flow (`/v1/auth/account/request` + `/v1/auth/account/response`) — generates ephemeral keypair, displays QR code (`easycoder:///account?[base64url-publicKey]`), user scans with existing EasyCoder mobile app to approve, receives encrypted account secret
- **Credential storage**: `~/.easycoder/agent.key` (separate from easycoder-cli's `~/.easycoder/access.key`)
- **Encryption**: AES-256-GCM (dataKey) for all new sessions. The master content keypair is derived deterministically from the account secret via `deriveKey(secret, 'EasyCoder EnCoder', ['content'])` → seed → `crypto_box_seed_keypair(seed)`. Per-session random keys are encrypted with the master public key and stored on the server.
...
- **Integration tests**: Use the real environment bootstrap (`yarn env:up:authenticated`) and exercise the live server + daemon + CLI stack. Do not use mocked acceptance coverage for `easycoder-agent spawn`.
...
- [x] Create `packages/easycoder-agent/` directory with `package.json` (name: `easycoder-agent`, type: module, bin: `./bin/easycoder-agent.mjs`)
- [x] Create `tsconfig.json` with strict mode, path aliases (`@/` → `src/`), ESM output
- [x] Create `bin/easycoder-agent.mjs` entry point wrapper (mirrors easycoder-cli pattern: spawns node with `--no-warnings`)
...
- [x] Implement key derivation tree:
  ...
  - `deriveContentKeyPair(secret)` — calls `deriveKey(secret, 'EasyCoder EnCoder', ['content'])` → seed → `sha512(seed)[0:32]` → `tweetnacl.box.keyPair.fromSecretKey()` → returns `{ publicKey, secretKey }`
...
- [x] Create `src/config.ts` — reads `EASYCODER_SERVER_URL` (default: `https://codeapi.daima.club`), `EASYCODER_HOME_DIR` (default: `~/.easycoder`), derives credential file path as `${happyHomeDir}/agent.key`
- [x] Create `src/credentials.ts`:
  ...
  - `readCredentials(config)` — parses `~/.easycoder/agent.key` JSON `{ token, secret }`, decodes secret from base64, derives contentKeyPair via `deriveContentKeyPair(secret)`. Returns `Credentials` or `null` if file missing.
  - `writeCredentials(config, token, secret)` — writes `{ token, secret: base64(secret) }` to `~/.easycoder/agent.key`
  - `clearCredentials(config)` — deletes `~/.easycoder/agent.key`
  - `requireCredentials(config)` — calls `readCredentials`, throws with "Run `easycoder-agent auth login` first" if null
...
### Task 4: Authentication command (`easycoder-agent auth`)
- [x] Create `src/auth.ts` implementing the account auth flow:
  ...
  3. Generate QR code data: `easycoder:///account?` + base64url(keypair.publicKey)
  4. Display QR code in terminal using `qrcode-terminal`
  5. Print instructions: "Scan this QR code with the EasyCoder app (Settings → Account → Link New Device)"
...
- [x] Add `easycoder-agent auth login` subcommand that runs the flow above
- [x] Add `easycoder-agent auth logout` subcommand that calls `clearCredentials()`
- [x] Add `easycoder-agent auth status` subcommand that reads credentials and prints auth status (authenticated / not authenticated)
...
- [x] Create `src/index.ts` using `commander` with program name `easycoder-agent`
- [x] `easycoder-agent list` — calls `listSessions`, displays table: ID (truncated), name/summary, path, status (active/inactive), last active time. With `--json` outputs raw JSON. With `--active` filters to active only.
- [x] `easycoder-agent status <session-id>` — fetches session via list + filter by ID prefix, connects Socket.IO to get live state, displays: session ID, metadata (path, host, lifecycle state), agent state (idle/busy, pending requests count), last message preview. With `--json` outputs raw JSON. Disconnects after displaying.
...
- [x] `easycoder-agent create --tag <tag> [--path <path>]` — creates new session with given tag and metadata (path defaults to cwd, host to hostname). Prints session ID. With `--json` outputs full session JSON.
- [x] `easycoder-agent send <session-id> <message>` — resolves session key, connects Socket.IO, sends user message (encrypted with AES-256-GCM), optionally waits for idle with `--wait`, and supports `--yolo` to send `permissionMode=yolo`. Disconnects after. Prints confirmation. With `--json` outputs message details.
...
- [x] `easycoder-agent history <session-id>` — fetches messages via HTTP, resolves session encryption key (dataKey or legacy), decrypts each message, displays in chronological order with role/timestamp. With `--json` outputs raw JSON. With `--limit <n>` limits output.
- [x] `easycoder-agent stop <session-id>` — connects Socket.IO, sends `session-end` event, disconnects. Prints confirmation.
- [x] `easycoder-agent wait <session-id> [--timeout <seconds>]` — connects Socket.IO, waits for agent idle state (no pending requests, not controlled by user), prints when idle or times out (default 300s). Exit code 0 on idle, 1 on timeout.
...
- [x] Verify interop: session created by easycoder-agent is visible and controllable from mobile app
- [x] Verify interop: session created by easycoder-cli can be listed and history read by easycoder-agent
- [x] Run full test suite (unit tests)
- [x] Run linter — all issues must be fixed

### Task 11: [Final] Update documentation
- [x] Add README.md to `packages/easycoder-agent/` with usage examples for all commands
- [x] Update root README if it references packages

### Task 12: Machines and spawn
- [x] Add `easycoder-agent machines [--active] [--json]`
- [x] Add machine record decryption using the existing account content key derivation and record data key pattern
- [x] Add `easycoder-agent spawn --machine <machine-id> [--path <path>] [--agent <agent>] [--create-dir] [--json]`
- [x] Reuse the existing machine RPC contract (`spawn-easycoder-session`) without encryption shortcuts
- [x] Add a real integration test that boots `yarn env:up:authenticated`, authenticates `easycoder-agent`, lists machines, spawns via the real daemon RPC path, and verifies the live session

## Technical Details

### CLI Commands Summary
```
easycoder-agent auth login                          # Authenticate via QR code (scanned by EasyCoder mobile app)
easycoder-agent auth logout                         # Clear stored credentials
easycoder-agent auth status                         # Show authentication status

easycoder-agent machines [--active] [--json]        # List machines
easycoder-agent list [--active] [--json]            # List all sessions
easycoder-agent spawn --machine <machine-id> [--path <path>] [--agent <agent>] [--create-dir] [--json]  # Spawn on a machine
easycoder-agent status <session-id> [--json]        # Get live session state
easycoder-agent create --tag <tag> [--path <path>] [--json]  # Create new session
easycoder-agent send <session-id> <message> [--yolo] [--wait] [--json]  # Send message
easycoder-agent history <session-id> [--limit <n>] [--json]    # Read message history
easycoder-agent stop <session-id>                   # Stop a session
easycoder-agent wait <session-id> [--timeout <s>]   # Wait for agent to become idle
```

### Authentication Flow (Account Auth)
```
easycoder-agent                          EasyCoder Server                    EasyCoder Mobile App
     |                                    |                               |
     ...
     |   easycoder:///account?[base64url-key] |                               |
     |                                    |                               |
     |                                    |  <-- User scans QR code ------+
     |                                    |                               |
     ...
     +-- Save { token, secret }           |                               |
     |   to ~/.easycoder/agent.key            |                               |
     |                                    |                               |
     +-- Derive content keypair:          |                               |
     |   deriveKey(secret,                |                               |
     |     'EasyCoder EnCoder', ['content'])  |                               |
     |   -> seed -> box keypair           |                               |
...
### Credential File Format (`~/.easycoder/agent.key`)
```json
{
...
```
secret (32 bytes)
  -> deriveKey(secret, 'EasyCoder EnCoder', ['content'])
  -> seed (32 bytes)
...
**For new sessions (created by easycoder-agent):**
...
**For existing sessions (created by easycoder-cli or other clients):**
...
## Post-Completion
**Manual verification:**
- Test full auth flow: run `easycoder-agent auth login`, scan QR with EasyCoder app, verify credentials saved
...
- Test `history` command for sessions created by both `easycoder-agent` and `easycoder-cli`
- Test cross-client interop: messages from easycoder-agent readable by mobile app and vice versa

**Distribution:**
- Package can be published to npm as `easycoder-agent`
- Alternatively, users install from monorepo via `yarn workspace easycoder-agent build`
