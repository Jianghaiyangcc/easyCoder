# Multi-process easycoder-server
...
> **Status:** the code in this doc is on `main` but `easycoder-server.yaml` ships
...
```
.
├── packages/easycoder-server/sources/app/
...
│
└── packages/easycoder-server/deploy/easycoder-server.yaml  k8s Deployment + Service
