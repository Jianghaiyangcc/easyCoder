...
This document describes the EasyCoder backend structure as implemented in `packages/easycoder-server`. It focuses on how the server is wired, how data flows through the system, and which subsystems handle which responsibilities.
...
## Process lifecycle
Entry point: `packages/easycoder-server/sources/main.ts`.
...
- API server: `packages/easycoder-server/sources/app/api/api.ts`
- Socket server: `packages/easycoder-server/sources/app/api/socket.ts`
- Event routing: `packages/easycoder-server/sources/app/events/eventRouter.ts`
- Presence: `packages/easycoder-server/sources/app/presence`
- Storage: `packages/easycoder-server/sources/storage`
- Prisma schema: `packages/easycoder-server/prisma/schema.prisma`
