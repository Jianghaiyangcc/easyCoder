...
```bash
minikube service easycoder-server --url
```

Use the printed URL as `SERVER_URL` for the tests below. If port-forwarding instead:

```bash
kubectl port-forward svc/easycoder-server 3005:3000
...
# Tail server logs
kubectl logs -l app=easycoder-server --all-containers -f
...
# Access Redis CLI
kubectl exec -it easycoder-redis-0 -- redis-cli
