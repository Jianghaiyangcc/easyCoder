...
cleanup() {
    ...
    pkill -f "port-forward svc/easycoder-server" 2>/dev/null || true
    pkill -f "port-forward svc/easycoder-redis" 2>/dev/null || true
}
trap cleanup EXIT
...
kubectl wait --for=condition=ready pod -l app=easycoder-server --timeout=120s
PODS=$(kubectl get pods -l app=easycoder-server --no-headers | wc -l | tr -d ' ')
echo -e "${GREEN}$PODS server pods ready${NC}"

# --- Server URL: use minikube tunnel for NodePort (real LB, survives pod kills) ---
setup_server_url() {
    local svc_type
    svc_type=$(kubectl get svc easycoder-server -o jsonpath='{.spec.type}' 2>/dev/null)
    ...
    if [ "$svc_type" = "NodePort" ]; then
        ...
        local tmpfile
        tmpfile=$(mktemp)
        minikube service easycoder-server --url > "$tmpfile" 2>/dev/null &
        PF_PIDS+=($!)
        sleep 2
        ...
        kubectl port-forward svc/easycoder-server 3005:3000 &>/dev/null &
        PF_PIDS+=($!)
    ...
    kubectl port-forward svc/easycoder-redis "$REDIS_PORT:6379" &>/dev/null &
    PF_PIDS+=($!)
...
wait_for_pods() {
    ...
    for i in {1..60}; do
        READY=$(kubectl get pods -l app=easycoder-server --no-headers 2>/dev/null | grep -c "Running" || true)
        if [ "$READY" -ge 2 ]; then
            break
        fi
        sleep 1
    done
    kubectl wait --for=condition=ready pod -l app=easycoder-server --timeout=120s 2>/dev/null || true
}
