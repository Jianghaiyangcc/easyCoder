...
function getLatestStableTag() {
  const tagsRaw = execSync(
    `git tag --list "easycoder-agent-v*" "happy-agent-v*" --sort=-v:refname`,
    {
      encoding: "utf8",
    }
  );
  ...
  // Prefer easycoder-agent-vX.Y.Z; keep happy-agent-vX.Y.Z as backward-compatible fallback.
  const stableTag = tags.find(
    (t) =>
      /^easycoder-agent-v\d+\.\d+\.\d+$/.test(t) ||
      /^happy-agent-v\d+\.\d+\.\d+$/.test(t)
  );
...
async function generateReleaseNotes() {
  try {
    ...
    try {
      gitLog = execSync(
        `git log ${commitRange} --pretty=format:"%h - %s (%an, %ar)" --no-merges -- packages/easycoder-agent`,
        { encoding: "utf8" }
      );
    } catch (error) {
      console.error(
      ...
      gitLog = execSync(
        `git log -10 --pretty=format:"%h - %s (%an, %ar)" --no-merges -- packages/easycoder-agent`,
        { encoding: "utf8" }
      );
    }
...
The release should cover commits since the latest stable easycoder-agent tag (easycoder-agent-vX.Y.Z): ${
      fromTag ?? "(none)"
    }.
