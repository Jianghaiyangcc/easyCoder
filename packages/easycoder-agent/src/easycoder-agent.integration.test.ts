...
async function listDaemonSessions(httpPort: number): Promise<Array<{ easycoderSessionId: string; pid: number; startedBy: string }>> {
    ...
    const parsed = await response.json() as { children: Array<{ easycoderSessionId: string; pid: number; startedBy: string }> };
...
describe('easycoder-agent integration', { timeout: 180_000 }, () => {
    ...
    it('authenticates, lists machines, and spawns a session through the real daemon RPC path', async () => {
        ...
        await waitFor(async () => {
            ...
            return sessions.some(session => session.easycoderSessionId === sessionId);
