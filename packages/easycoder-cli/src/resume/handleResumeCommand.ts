...
import { resolveEasycoderSession, type ResumableEasycoderSession } from './resolveEasycoderSession';
...
export function buildResumeLaunch(session: ResumableEasycoderSession, options: ResumeLaunchOptions = {}): ResumeLaunch {
...
function spawnResumeChild(launch: ResumeLaunch): Promise<number | null> {
    return new Promise((resolve, reject) => {
        const child = spawnEasycoderCLI(launch.args, {
            cwd: launch.cwd,
            env: process.env,
            stdio: 'inherit',
        });
...
export async function handleResumeCommand(args: string[]): Promise<void> {
    ...
    const session = await resolveEasycoderSession(parsed.sessionId);
