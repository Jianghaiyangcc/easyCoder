...
export async function handleSandboxStatus(): Promise<void> {
    ...
    if (!config) {
        console.log('Sandbox is not configured. Run `easycoder sandbox configure`.');
        return;
    }
...
export function handleSandboxHelp(): void {
    console.log(`
${chalk.bold('easycoder sandbox')} - Sandbox management

${chalk.bold('Usage:')}
  easycoder sandbox configure      Configure sandbox settings interactively
  easycoder sandbox status         Show current sandbox configuration
  easycoder sandbox disable        Disable sandboxing
  easycoder sandbox help           Show this help
`);
}
