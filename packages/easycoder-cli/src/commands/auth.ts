...
function showAuthHelp(): void {
  console.log(`
${chalk.bold('easycoder auth')} - Authentication management

${chalk.bold('Usage:')}
  easycoder auth login [--force]    Authenticate with EasyCoder
  easycoder auth logout             Remove authentication and machine data
  easycoder auth status             Show authentication status
  easycoder auth help               Show this help message
...
async function handleAuthLogin(args: string[]): Promise<void> {
  ...
  if (!forceAuth) {
    ...
    if (existingCreds && settings?.machineId) {
      ...
      console.log(chalk.gray(`  Use 'easycoder auth login --force' to re-authenticate`));
      return;
...
async function handleAuthLogout(): Promise<void> {
  ...
  const happyDir = configuration.easycoderHomeDir;
  ...
  console.log(chalk.blue('This will log you out of EasyCoder'));
  console.log(chalk.yellow('⚠️  You will need to re-authenticate to use EasyCoder again'));
  ...
  if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
    try {
      ...
      // Remove entire easycoder directory (as current logout does)
      if (existsSync(happyDir)) {
        rmSync(happyDir, { recursive: true, force: true });
      }

      console.log(chalk.green('✓ Successfully logged out'));
      console.log(chalk.gray('  Run "easycoder auth login" to authenticate again'));
    } catch (error) {
...
async function handleAuthStatus(): Promise<void> {
  ...
  if (!credentials) {
    ...
    console.log(chalk.gray('  Run "easycoder auth login" to authenticate'));
    return;
  }
  ...
  if (settings?.machineId) {
    ...
    console.log(chalk.gray('  Run "easycoder auth login --force" to fix this'));
  }

  // Data location
  console.log(chalk.gray(`\n  Data directory: ${configuration.easycoderHomeDir}`));
