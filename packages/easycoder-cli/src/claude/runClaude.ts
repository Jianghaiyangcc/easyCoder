...
import { startEasycoderServer } from '@/claude/utils/startEasycoderServer';
...
export async function runClaude(credentials: Credentials, options: StartOptions = {}): Promise<void> {
    ...
    if (!machineId) {
        console.error(`[START] No machine ID found in settings, which is unexpected since authAndSetupMachineIfNeeded should have created it. Please report this issue on https://github.com/Jianghaiyangcc/easyCoder-cli/issues`);
        process.exit(1);
    }
    ...
    let metadata: Metadata = {
        ...
        easycoderToolsDir: resolve(projectPath(), 'tools', 'unpacked'),
    ...
    const easycoderServer = await startEasycoderServer(session);
    logger.debug(`[START] EasyCoder MCP server started at ${easycoderServer.url}`);
    ...
    const exitCode = await loop({
        ...
        messageQueue,
        api,
        allowedTools: easycoderServer.toolNames.map(toolName => `mcp__happy__${toolName}`),
