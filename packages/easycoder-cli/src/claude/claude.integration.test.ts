 ...
 *   - EasyCoder MCP tool usage (`mcp__easycoder__change_title`)
...
import { startEasycoderServer } from './utils/startEasycoderServer';
...
class ClaudeQueryDriver {
    private easycoderServer: Awaited<ReturnType<typeof startEasycoderServer>> | null = null;
    ...
    async start(): Promise<void> {
        ...
        this.easycoderServer = await startEasycoderServer(fakeSessionClient);
    }

    stop(): void {
        this.easycoderServer?.stop();
        this.easycoderServer = null;
    }
...
describe.skipIf(!claudeAvailable)('Claude Integration (SDK/query)', { timeout: 180_000 }, () => {
    ...
    it('should clarify natively, resume across a model switch, use TodoWrite, and cross the project boundary with native tools', async () => {
        ...
        const execution = await driver!.runTurn({
            allowedTools: ['mcp__easycoder__change_title', 'TodoWrite', 'TodoRead', 'Write', 'Edit', 'Read', 'Glob', 'LS'],
        ...
        expect(execution.toolUseNames).toContain('mcp__easycoder__change_title');
    ...
    it('should leave the file untouched and explain the refusal when native write is explicitly disallowed', async () => {
        const denied = await driver!.runTurn({
            allowedTools: ['mcp__easycoder__change_title', 'Write', 'Edit', 'Read', 'LS'],
