...
async function stopAllTrackedSessions(): Promise<void> {
  ...
  await Promise.all(
    sessions.map((session: any) =>
      stopDaemonSession(session.easycoderSessionId ?? `PID-${session.pid}`).catch(() => false)
    ),
  );
}
...
describe('Daemon Integration Tests', { timeout: 180_000 }, () => {
  ...
  beforeEach(async () => {
    ...
    void spawnEasycoderCLI(['daemon', 'start'], {
      stdio: 'ignore',
    });
  ...
  it('should track session-started webhook from terminal session', async () => {
    ...
    const mockMetadata: Metadata = {
      ...
      host: 'test-host',
      homeDir: '/test/home',
      easycoderHomeDir: '/test/happy-home',
      easycoderLibDir: '/test/happy-lib',
      easycoderToolsDir: '/test/happy-tools',
      hostPid: 99999,
      startedBy: 'terminal',
    ...
    expect(tracked.easycoderSessionId).toBe('test-session-123');
  ...
  it('should spawn & stop a session via HTTP (not testing RPC route, but similar enough)', async () => {
    ...
    const spawnedSession = sessions.find(
      (s: any) => s.easycoderSessionId === response.sessionId
    );
    ...
    expect(spawnedSession.easycoderSessionId).toBeDefined();
    await stopDaemonSession(spawnedSession.easycoderSessionId);
  });
  ...
  it('should track both daemon-spawned and terminal sessions', async () => {
    ...
    const terminalHappyProcess = spawnEasycoderCLI([
    ...
    const daemonSession = sessions.find(
      (s: any) => s.easycoderSessionId === spawnResponse.sessionId
    );
    ...
    await stopDaemonSession(daemonSession.easycoderSessionId);
  ...
  it('should update session metadata when webhook is called', async () => {
    ...
    const sessions = await listDaemonSessions();
    const session = sessions.find((s: any) => s.easycoderSessionId === spawnResponse.sessionId);
  ...
  it('should handle concurrent session operations', async () => {
    ...
    const daemonSessions = sessions.filter(
      (s: any) => s.startedBy === 'daemon' && spawnedSessionIds.includes(s.easycoderSessionId)
    );
    expect(daemonSessions.length).toBeGreaterThanOrEqual(3);

    // Stop all spawned sessions
    for (const session of daemonSessions) {
      expect(session.easycoderSessionId).toBeDefined();
      await stopDaemonSession(session.easycoderSessionId);
    }
  });
   ...
   * 6. Daemon spawns new daemon via spawnEasycoderCLI(['daemon', 'start'])
