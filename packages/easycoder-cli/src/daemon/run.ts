...
import { spawnEasycoderCLI } from '@/utils/spawnEasycoderCLI';
...
import { detectResumeSupport } from '@/resume/localEasycoderAgentAuth';
import { resolveEasycoderSession } from '@/resume/resolveEasycoderSession';
...
export async function startDaemon(): Promise<void> {
  ...
  let requestShutdown: (source: 'easycoder-app' | 'easycoder-cli' | 'os-signal' | 'exception', errorMessage?: string) => void;
  let resolvesWhenShutdownRequested = new Promise<({ source: 'easycoder-app' | 'easycoder-cli' | 'os-signal' | 'exception', errorMessage?: string })>((resolve) => {
  ...
  try {
    ...
    const onHappySessionWebhook = (sessionId: string, sessionMetadata: Metadata) => {
      ...
      if (existingSession && existingSession.startedBy === 'daemon') {
        ...
        existingSession.easycoderSessionId = sessionId;
      ...
      try {
        ...
        if (useTmux && tmuxSessionName !== undefined) {
          ...
          if (tmuxResult.success) {
            ...
            // Wait for webhook to populate session with easycoderSessionId (exact same as regular flow)
            ...
            return new Promise((resolve) => {
              ...
              pidToAwaiter.set(tmuxResult.pid!, (completedSession) => {
                ...
                logger.debug(`[DAEMON RUN] Session ${completedSession.easycoderSessionId} fully spawned with webhook (tmux)`);
                resolve({
                  type: 'success',
                  sessionId: completedSession.easycoderSessionId!
                });
              });
            });
          } else {
    ...
    const spawnTrackedHappyProcess = ({
      ...
      const happyProcess = spawnEasycoderCLI(args, {
        cwd,
        detached: true,
        stdio: 'ignore',
        env,
      ...
      return new Promise((resolve) => {
        ...
        pidToAwaiter.set(happyProcess.pid!, (completedSession) => {
          clearTimeout(timeout);
          logger.debug(`[DAEMON RUN] Session ${completedSession.easycoderSessionId} fully spawned with webhook`);
          resolve({
            type: 'success',
            sessionId: completedSession.easycoderSessionId!
          });
    ...
    const resumeSession = async (easycoderSessionId: string): Promise<SpawnSessionResult> => {
      try {
        const previousSession = await resolveEasycoderSession(easycoderSessionId);
    ...
    const stopSession = (sessionId: string): boolean => {
      ...
      for (const [pid, session] of pidToTrackedSession.entries()) {
        if (session.easycoderSessionId === sessionId ||
    ...
    const { port: controlPort, stop: stopControlServer } = await startDaemonControlServer({
      ...
      stopSession,
      spawnSession,
      requestShutdown: () => requestShutdown('easycoder-cli'),
    ...
    apiMachine.setRPCHandlers({
      ...
      resumeSession,
      stopSession,
      requestShutdown: () => requestShutdown('easycoder-app')
    });
    ...
    const restartOnStaleVersionAndHeartbeat = setInterval(async () => {
      ...
      if (projectVersion !== configuration.currentCliVersion) {
        ...
        // Done!
        try {
          spawnEasycoderCLI(['daemon', 'start'], {
            detached: true,
            stdio: 'ignore'
          });
        } catch (error) {
    ...
    // Setup signal handlers
    const cleanupAndShutdown = async (source: 'easycoder-app' | 'easycoder-cli' | 'os-signal' | 'exception', errorMessage?: string) => {
