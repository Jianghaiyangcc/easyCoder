...
async function daemonPost(path: string, body?: any): Promise<{ error?: string } | any> {
  ...
  try {
    const timeout = process.env.EASYCODER_DAEMON_HTTP_TIMEOUT ? parseInt(process.env.EASYCODER_DAEMON_HTTP_TIMEOUT) : 10_000;
 ...
 * For instance we are not handling the case where we upgraded easycoder,
 ...
 * For instance when running `easycoder daemon status` we can show more information.
 */
...
export async function isDaemonRunningCurrentlyInstalledHappyVersion(): Promise<boolean> {
  ...
  try {
    ...
    // we will get a new path or not when easycoder is upgraded globally.
    ...
    /*
    const { spawnEasycoderCLI } = await import('@/utils/spawnEasycoderCLI');
    const happyProcess = spawnEasycoderCLI(['--version'], { stdio: 'pipe' });
