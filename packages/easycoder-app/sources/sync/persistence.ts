...
export type NewSessionAgentType = 'claude' | 'codex' | 'gemini' | 'openclaw' | 'opencode';
...
export function loadThemePreference(): 'light' | 'dark' | 'zinc' | 'midnight' | 'claude' | 'adaptive' {
    const localSettings = mmkv.getString('local-settings');
    if (localSettings) {
        try {
            const parsed = JSON.parse(localSettings);
...
export function loadNewSessionDraft(): NewSessionDraft | null {
    ...
    try {
        ...
        const agentType: NewSessionAgentType = parsed.agentType === 'codex' || parsed.agentType === 'gemini' || parsed.agentType === 'openclaw' || parsed.agentType === 'opencode'
            ? parsed.agentType
            : 'claude';
