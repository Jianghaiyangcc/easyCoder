...
import { LocalSettings, applyLocalSettings, localSettingsDefaults } from "./localSettings";
...
export const storage = create<StorageState>()((set, get) => {
    ...
    return {
        ...
        applySessions: (sessions: (Omit<Session, 'presence'> & { presence?: "online" | number })[]) => set((state) => {
            // Load drafts, permission modes, model modes, and effort levels if sessions are empty (initial load)
            const isInitialLoad = Object.keys(state.sessions).length === 0;
            const savedDrafts = isInitialLoad ? loadSessionDrafts() : {};
            const savedPermissionModes = isInitialLoad ? loadSessionPermissionModes() : {};
            const savedModelModes = isInitialLoad ? loadSessionModelModes() : {};
            const savedEffortLevels = isInitialLoad ? loadSessionEffortLevels() : {};
            ...
            sessions.forEach(session => {
                ...
                // Preserve existing model mode if it exists, or load from saved data
                ...
                // Preserve existing effort level if it exists, or load from saved data
                const existingEffortLevel = state.sessions[session.id]?.effortLevel;
                const savedEffortLevel = savedEffortLevels[session.id];
                const resolvedEffortLevel = existingEffortLevel || savedEffortLevel || session.effortLevel || null;
        ...
        updateSessionModelMode: (sessionId: string, mode: string) => set((state) => {
            ...
            // Collect all model modes for persistence
        ...
        updateSessionEffortLevel: (sessionId: string, level: string) => set((state) => {
            ...
            // Collect all effort levels for persistence
            const allEffortLevels: Record<string, string> = {};
            Object.entries(updatedSessions).forEach(([id, sess]) => {
                if (sess.effortLevel && sess.effortLevel !== 'default') {
                    allEffortLevels[id] = sess.effortLevel;
                }
            });

            // Persist effort levels (only non-default values to save space)
...
export function resetStorageForLogout() {
