...
import { payments } from './payments/payments'; // New payment system
...
class Sync {
    ...
    private unsubscribeUpdateMessages: (() => void) | null = null;
    private unsubscribeEphemeralMessages: (() => void) | null = null;
    private unsubscribeReconnected: (() => void) | null = null;
    ...
    private unsubscribeFromUpdates() {
        this.unsubscribeUpdateMessages?.();
        this.unsubscribeUpdateMessages = null;
        this.unsubscribeEphemeralMessages?.();
        this.unsubscribeEphemeralMessages = null;
        this.unsubscribeReconnected?.();
        this.unsubscribeReconnected = null;
    }
    ...
    async sendMessage(sessionId: string, text: string, options?: SendMessageOptions) {
        ...
        const kind: 'text' | 'voice' = source === 'voice' ? 'voice' : 'text';
    ...
    presentPaywall = async (flow?: string): Promise<{ success: boolean; purchased?: boolean; error?: string }> => {
        try {
            // Try to use new payment system first
            if (this.serverID) {
                // Initialize payments if not already done
                try {
                    await payments.initialize(this.serverID);

                    // Present paywall using new system
                    const result = await payments.presentPaywall(
                    ...
                    console.log('[Paywall] New payment system failed, falling back to RevenueCat:', error);
                    // Continue to RevenueCat fallback
                }
            }

            // Fallback to RevenueCat if new system fails or not initialized
            if (!this.revenueCatInitialized) {
    ...
    private subscribeToUpdates = () => {
        ...
        this.unsubscribeUpdateMessages = apiSocket.onMessage('update', this.handleUpdate.bind(this));
        this.unsubscribeEphemeralMessages = apiSocket.onMessage('ephemeral', this.handleEphemeralUpdate.bind(this));

        // Subscribe to connection state changes
        this.unsubscribeReconnected = apiSocket.onReconnected(() => {
    ...
    async destroy() {
        ...
        this.backgroundSendStartedAt = null;
        this.recalculationLockCount = 0;
        this.lastRecalculationTime = 0;
        this.activityAccumulator.reset();
        this.encryptionCache = new EncryptionCache();

        this.credentials = undefined as unknown as AuthCredentials;
        this.encryption = undefined as unknown as Encryption;
        this.serverID = '';
        this.anonID = '';
...
export async function syncDestroy() {
...
async function syncInit(credentials: AuthCredentials, restore: boolean) {
    ...
    unsubscribeSocketStatus?.();
    unsubscribeSocketStatus = apiSocket.onStatusChange((status) => {
