...
export function useConnectTerminal(options?: UseConnectTerminalOptions) {
    ...
    const processAuthUrl = React.useCallback(async (url: string) => {
        if (!url.startsWith('easycoder://terminal?')) {
        ...
        setIsLoading(true);
        try {
            const tail = url.slice('easycoder://terminal?'.length);
    ...
    const connectTerminal = React.useCallback(async () => {
        ...
        try {
            ...
            try {
                console.log('[Scanner] Launching terminal QR scanner', {
                ...
                console.error('Failed to launch terminal scanner:', error);
    ...
    React.useEffect(() => {
        if (CameraView.isModernBarcodeScannerAvailable) {
            const subscription = CameraView.onModernBarcodeScanned(async (event) => {
                ...
                if (event.data.startsWith('easycoder://terminal?')) {
