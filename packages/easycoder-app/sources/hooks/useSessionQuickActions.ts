...
export function useSessionQuickActions(
    ...
    const [resumingSession, performResume] = useHappyAction(async () => {
        if (!resumeAvailability.canResume) {
            throw new EasycoderError(resumeAvailability.message, false);
        }

        if (!machineId) {
            throw new EasycoderError(t('sessionInfo.resumeSessionMissingMachine'), false);
        }
        ...
        switch (result.type) {
            ...
            case 'requestToApproveDirectoryCreation':
                throw new EasycoderError(t('sessionInfo.resumeSessionUnexpectedDirectoryPrompt'), false);
            case 'error':
                throw new EasycoderError(result.errorMessage, false);
        }
    });
