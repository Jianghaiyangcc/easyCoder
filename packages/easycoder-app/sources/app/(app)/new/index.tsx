...
function NewSessionScreen() {
        ...
        try {
            ...
            switch (result.type) {
                case 'success':
                    ...
                    // Set permission mode, model, and effort level on the session before sending
                    ...
                    if (currentEffortKey) {
                        storage.getState().updateSessionEffortLevel(result.sessionId, currentEffortKey);
                    }
    ...
    }, [selectedMachineId, selectedMachine, selectedPath, selectedAgent, prompt, router, navigateToSession, currentPermission.key, currentModelKey, currentEffortKey, worktreeKey]);
                        ...
                        {isConfigExpanded ? (
                            ...
                            <>
                                ...
                                <View style={styles.collapsedIconsRow}>
                                    ...
                                    <Pressable
                                        onPress={() => {
                                            const currentIndex = availableAgents.findIndex(a => a.key === selectedAgent);
                                            const nextAgent = availableAgents.length > 0
                                                ? availableAgents[(currentIndex + 1) % availableAgents.length]
                                                : null;
                                            cycleAgent();
                                            if (nextAgent) {
