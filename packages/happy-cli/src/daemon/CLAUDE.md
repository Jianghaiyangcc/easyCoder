# EasyCoder CLI Daemon: Control Flow and Lifecycle

The daemon is a persistent background process that manages EasyCoder sessions, enables remote control from the mobile app, and handles auto-updates when the CLI version changes.
...
4. `startDaemon()` performs startup:
   ...
   - RPC registration: exposes `spawn-easycoder-session`, `stop-session`, `requestShutdown` handlers
   - Heartbeat loop: every 60s (or EASYCODER_DAEMON_HEARTBEAT_INTERVAL) checks for version updates and prunes dead sessions
...
The daemon detects when `npm upgrade easycoder` occurs:
...
1. Backend forwards RPC `spawn-easycoder-session` to daemon via WebSocket
2. `ApiMachineClient` invokes `spawnSession()` handler
3. `spawnSession()`:
   - Creates directory if needed
   - Spawns detached EasyCoder process with `--easycoder-starting-mode remote --started-by daemon`
   - Adds to `pidToTrackedSession` map
   - Sets up 10-second awaiter for session webhook
4. New EasyCoder process:
...
User runs `easycoder` directly:
1. CLI auto-starts daemon if configured
2. EasyCoder process calls `notifyDaemonSessionStarted()` 
3. Daemon receives webhook, creates `TrackedSession` with `startedBy: 'easycoder directly...'`
...
`easycoder doctor` uses `ps aux | grep` to find all EasyCoder processes:
- Production: matches `easycoder.mjs`, `easycoder` (or legacy `happy-coder`), `dist/index.mjs`
...
- Server to Daemon: rpc-request (spawn-easycoder-session, stop-session, requestShutdown)
...
```json
{
  ...
  "metadata": "base64(encrypted({
    ...
    'homeDir': '/Users/john',
    'happyHomeDir': '/Users/john/.easycoder'
  }))",
