...
function buildEnvVars(envDir: string, serverPort: number, expoPort: number): Record<string, string> {
    ...
    return {
        ...
        EXPO_PUBLIC_EASYCODER_SERVER_URL: `http://localhost:${serverPort}`,
        EXPO_PUBLIC_LOG_SERVER_URL: "http://localhost:8787",
        EXPO_PORT: String(expoPort),

        // CLI
        EASYCODER_SERVER_URL: `http://localhost:${serverPort}`,
        EASYCODER_WEBAPP_URL: `http://localhost:${expoPort}`,
        EASYCODER_HOME_DIR: path.join(envDir, "cli", "home"),
        EASYCODER_PROJECT_DIR: projectDir,
        EASYCODER_VARIANT: "dev",
        DEBUG: "1",
        ...(devAuth ? {
...
function buildEnvSh(name: string, envDir: string, serverPort: number, expoPort: number): string {
    ...
    const lines: string[] = [
        `# EasyCoder Dev Environment: ${name}`,
    ...
    lines.push(`export EXPO_PUBLIC_EASYCODER_SERVER_URL="${vars.EXPO_PUBLIC_EASYCODER_SERVER_URL}"`);
    ...
    lines.push("# CLI");
    lines.push(`export EASYCODER_SERVER_URL="${vars.EASYCODER_SERVER_URL}"`);
    lines.push(`export EASYCODER_WEBAPP_URL="${vars.EASYCODER_WEBAPP_URL}"`);
    lines.push(`export EASYCODER_HOME_DIR="${vars.EASYCODER_HOME_DIR}"`);
    lines.push(`export EASYCODER_PROJECT_DIR="${vars.EASYCODER_PROJECT_DIR}"`);
    lines.push(`export EASYCODER_VARIANT=dev`);
    lines.push(`export DEBUG=1`);
    lines.push(`export PATH="${path.join(envDir, "bin")}:$PATH"`);
    lines.push("");
    lines.push("# Commands exposed by this env");
    lines.push("# - easycoder");
    lines.push("# - easycoder-agent");
    lines.push("");
...
function writeEnvCommands(envDir: string): void {
    ...
    const commands = [
        ...
        },
        {
            ...
            entrypoint: path.join(REPO_ROOT, "packages", "easycoder-agent", "bin", "easycoder-agent.mjs"),
        },
    ];
...
function buildCliCommand(envDir: string): string {
    return `source "${path.join(envDir, "env.sh")}" && easycoder`;
}
...
async function main(): Promise<void> {
    ...
    switch (subcommand) {
        ...
        case "tailscale":
            ...
            break;
        default:
            console.log(`EasyCoder Environment Manager
