...
## CLI Release
    ...
    Package:     packages/easycoder-cli
    npm name:    easycoder-cli
...
1. `npm view easycoder-cli dist-tags` — see current latest + beta
2. `cat packages/easycoder-cli/package.json | grep version` — local version
...
Edit `packages/easycoder-cli/package.json` directly — do NOT use `npm version` (it chokes on pnpm workspace protocol).
...
pnpm --filter easycoder-cli run build
```
...
```bash
cd packages/easycoder-cli
pnpm --filter easycoder-cli exec vitest run --project unit
```
...
```bash
npm view easycoder-cli dist-tags
```
...
## Mobile Release
    ...
    Package:     packages/easycoder-app
...
- **OTA update (preview)** — push JS update to preview channel (most common)
  ```bash
  pnpm --filter easycoder-app run ota
  ```

- **OTA update (production)** — push JS update to production channel
  ```bash
  pnpm --filter easycoder-app run ota:production
  ```
...
- **Dev builds** — development + preview variants (internal distribution)
  ```bash
  pnpm --filter easycoder-app run release:build:developer
  ```

- **App Store** — production builds with auto-submit
  ```bash
  pnpm --filter easycoder-app run release:build:appstore
  ```
...
## Web Release
    ...
    Package:     packages/easycoder-app (same Expo app, web export)
    Dockerfile:  Dockerfile.webapp
    Image:       docker.korshakov.com/easycoder-app:{version}
    K8s:         packages/easycoder-app/deploy/easycoder-app.yaml (3 replicas)

Web releases go through TeamCity (`Lab_EasyCoderWeb`). The config is in the TeamCity UI, not in the repo.
...
## Server Release
    ...
    Package:     packages/easycoder-server
    Dockerfile:  Dockerfile.server (production), Dockerfile (standalone w/ PGlite)
    Image:       docker.korshakov.com/easycoder-server:{version}
    K8s:         packages/easycoder-server/deploy/easycoder-server.yaml (1 replica, port 3005)

Server releases go through TeamCity (`Lab_EasyCoderServer`). The config is in the TeamCity UI, not in the repo.

Build: node:20 + python3 + ffmpeg, builds easycoder-wire + easycoder-server.
Secrets from Vault: handy-db, handy-master, handy-github, handy-files, handy-e2b, handy-revenuecat, handy-elevenlabs.
Redis: easycoder-redis StatefulSet (redis:7-alpine, 1Gi persistent volume).
